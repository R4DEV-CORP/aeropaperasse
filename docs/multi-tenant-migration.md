...
**Update (2026-06-01) — Livewire updates lost tenant context:** browser-reported `SQLSTATE[42S02] aeropaperasse_central.clients doesn't exist` when submitting the "create user account" modal from `coworkers/index` on tenant REM. Root cause: the web routes apply `InitializeTenancyByDomain` at the route-group level, but Livewire's `/livewire/update` endpoint goes through the `web` middleware group only and its "persistent middleware" allowlist (default: `Authenticate`, `SubstituteBindings`, …) doesn't include the tenancy classes — so every Livewire action ran with the default connection back on `central`. Tests didn't catch it because `TenantTestCase` initializes tenancy manually. **Fix:** `AppServiceProvider::boot()` now calls `Livewire::addPersistentMiddleware([InitializeTenancyByDomain, PreventAccessFromCentralDomains, EnsureTenantMembership])` so they're replayed from the originating route on every update. Full suite still **85/85 green**. See [gotchas](#poc-gotchas).

**Update (2026-06-01) — two follow-ups surfaced by Playwright E2E testing the same modal:**
- **Sanctum `personal_access_tokens` was unpinned:** the post-Livewire-fix login then 500'd on `tenant_rem.personal_access_tokens` because `AuthService::createWebSession()` calls `$user->createToken('web_token')`, and Sanctum's stock `PersonalAccessToken` model has no `$connection`. **Fix:** new `App\Models\PersonalAccessToken extends SanctumPersonalAccessToken` pinned to `central`, registered via `Sanctum::usePersonalAccessTokenModel(...)` in `AppServiceProvider::boot()`.
- **`unique:users` / `exists:users` validation rules were unqualified:** with tenancy initialized, the `unique:users,email` rule in `components/coworkers/⚡make-user-modal.blade.php` and `CoworkerValidator::validateComplete` / `::validateUserCreation` was running its `select count(*)` against `tenant_<id>.users` (the connection-aware DatabasePresenceVerifier picks the default connection). **Fix:** qualified to `unique:central.users,email` on the live path. Legacy API controllers left as-is.
- **Method:** validated end-to-end in the browser with Playwright (Sanctum login → coworkers/index → modal → submit → toast + list refresh, `Avec compte 0 → 1`). Going forward, browser-verify tenancy-touching Livewire flows — the unit/feature suite manually inits tenancy in `TenantTestCase::setUp()` and won't catch these.

**Update (2026-06-01) — page-by-page Playwright sweep, 2 cross-DB JOIN bugs fixed:** swept every live page on both `app.*` and `client1.*` as REM super-admin. All index/create/auth/chooser pages OK. **Bugs found & fixed:**
- **`/coworkers/{id}` and `/coworkers/form/{id}` (edit):** `Coworker::trainings()` belongsToMany emitted a JOIN between central `trainings` and tenant `coworker_trainings` (500 in both directions). Converted the three call sites (`coworkers/⚡show`, `coworkers/⚡form` mount + syncTrainings) to consume the **`coworkerTrainings` HasMany + `with('training')`** pattern — two separate queries, no JOIN. Also replaced the `DB::table('coworker_trainings')->insert/update/delete` in `syncTrainings` with `CoworkerTraining` Eloquent calls. Verified end-to-end in browser (seeded 2 `CoworkerTraining` rows with mixed valid/expired states → screenshot showed correctly rendered list with central title + tenant dates/airport).
- **`/trainings/{companyId}`:** the page built a `DB::table('coworker_trainings')->join('coworkers', …)->join('trainings', …)` — same cross-DB JOIN trap (tenant connection joining central). Rewritten as `CoworkerTraining::query()->with(['coworker', 'training'])->whereIn('coworker_id', …)`, ordering done in PHP for active section. View bindings switched from `$row->coworker_firstname` / `$row->training_title` to `$row->coworker?->firstname` / `$row->training?->title`.

Full suite still **85/85 green** after the refactor. See [gotchas](#poc-gotchas) for the two new patterns documented (no cross-DB belongsToMany; no `DB::table` JOIN across the boundary).

**Update (2026-06-01) — création de compte coworker = pivot manquante, nouveaux rôles assignables, auto-upgrade utilisateur existant :** trois corrections autour du flow "création coworker + utilisateur" :
- **Bug** : `CreateCoworkerAction` (page `/coworkers/form` cochée *Créer un compte*) et `⚡make-user-modal` (action sur un coworker existant) écrivaient bien la ligne `users` mais **n'ajoutaient pas la pivot `tenant_user`** → tous les comptes créés via ce flow atterrissaient sur `tenant.no-access` au login. Fix : pivot attachée systématiquement (skip si rôle REM, idempotent sur users déjà membres).
- **Rôles assignables** : élargis pour inclure `owner` et `tenant_admin`. Helper central `App\Enums\Role::assignableBy(User $actor)` (REM super admin = tout ; REM admin = sauf super admin ; owner = `tenant_admin` + scopes société ; tenant_admin = scopes société uniquement) consommé à la fois par les dropdowns (`⚡make-user-modal`, `pages/coworkers/⚡form`) **et** par les règles `in:` du `CoworkerValidator` — UI et serveur restent en sync.
- **Auto-upgrade utilisateur existant** : si l'email tapé correspond à un user central existant, le formulaire bascule en mode "rattacher" : masque les champs mot de passe, affiche le nom du user trouvé, et le submit crée le coworker + insère seulement la pivot (rôle + `client_id`). Bloqué si le user est déjà membre du tenant courant (sinon violerait `unique(user_id,tenant_id)`). DTO `CreateCoworkerData::existing_user_id` + `shouldAttachExistingUser()`. **La liste des autres tenants** du user existant est affichée **uniquement aux REM staff** (`@if ($authUser->isRemStaff())`) — un owner/tenant_admin ne doit pas voir d'info cross-tenant.

**Update (2026-06-01) — `can_access_formation` migré sur la pivot `tenant_user` :** le flag d'accès aux formations devient per-tenant. Migration centrale `2026_06_01_135236_add_can_access_formation_to_tenant_user` ajoute la colonne sur la pivot + backfill via `UPDATE … JOIN` depuis `users.can_access_formation` (snapshot du global). Nouvelle méthode contextuelle `User::canAccessFormation(?tenantId)` : REM staff court-circuitent à `true`, sinon pivot du tenant actif, fallback colonne dépréciée (parité avec `contextualClientId()`). `User::tenants()` étendu à `withPivot(['role','client_id','can_access_formation'])`. ~8 sites de lecture migrés (`layouts::app`, `coworkers/⚡show|⚡index|⚡form`, `trainings/⚡show|⚡index`, `trainings/⚡assign-modal|⚡upload-certificate-modal`) ; ~3 sites d'écriture (`CreateCoworkerAction`, `⚡make-user-modal`, `coworkers/⚡form::submitUpdate` via `updateExistingPivot`). Seeder + `userIdsWithFormationAccess()` réécrits pour interroger la pivot (REM staff inclus en union). La colonne `users.can_access_formation` reste physiquement en place mais devient dépréciée (même pattern que `users.client_id` — Q-CLIENT).

**Update (2026-06-01) — catalogue formations réel :** `PocMultitenantSeeder::seedSharedTrainings` charge maintenant `database/data/trainings.php` (10 formations réelles pré-multi-tenant : 11.2.6.2, 11.2.3.9/10 +/- TCA, Sécurité piétons, Permis T, Pratique permis T, Facteur humain, Co activité). `Permis T` reste seul avec `requires_airport = true` (cf. la migration originale `2026_04_29_092341`). Idempotent via `firstOrCreate`.
...
- **Central-pinned models** (`$connection = 'central'`): `User`, `Training`, `TwoFactorCode` (2FA is issued during login, which runs in tenant context), `App\Models\PersonalAccessToken` (extends `Laravel\Sanctum\PersonalAccessToken`, registered via `Sanctum::usePersonalAccessTokenModel()` in `AppServiceProvider`). `Tenant`/`Domain` are pinned by the package (`CentralConnection`). Sanctum's stock model has no `$connection`, so `$user->createToken('web_token')` during login 500'd on `tenant_<id>.personal_access_tokens` until the custom model was registered.
- **Validation `exists`/`unique` rules against central tables** must name the connection: `exists:central.trainings,id`, `unique:central.users,email` (a bare `exists:trainings,id` / `unique:users,email` would hit the tenant DB → `Base table not found: tenant_<id>.users`). Rules against tenant tables (`clients`, `coworkers`, …) stay unqualified. Already corrected on the live path: the make-user modal (`components/coworkers/⚡make-user-modal.blade.php`) and `CoworkerValidator::validateComplete` / `::validateUserCreation`. Legacy API controllers (`AuthController`, `ClientUserController`, `UserController`, `TrainingController`) still carry bare `unique:users` / `exists:users` and were left as-is.
...
- **No cross-DB `belongsToMany` between a central model and a tenant pivot.** `Coworker::trainings()` = `belongsToMany(Training::class, 'coworker_trainings')` emits a single SELECT with `INNER JOIN coworker_trainings` — but `Training` is pinned central so the query runs on the central connection while `coworker_trainings` lives in the tenant DB (and vice-versa from the central side). Both directions 500 (`Base table not found: central.coworker_trainings` or `tenant_X.trainings`). **Workaround (applied):** consume the central pivot model `CoworkerTraining` (tenant) with `with('training')` — Eloquent fetches `Training` rows by id in a separate query on the central connection (no JOIN). The vues `coworkers/show`, `coworkers/form`, `trainings/show` were converted off `with('trainings')` / `$cw->trainings` onto `with('coworkerTrainings.training')` / `$cw->coworkerTrainings`. Same pattern for any future central-pivot-tenant belongsToMany (e.g. `Client::trainings()`, `User::trainings()` — both still defined but unused on the live path; the latter targets the dead legacy `user_trainings` table).
- **`DB::table(...)` with a JOIN across the boundary is a trap of the same family.** `trainings/show` previously used `DB::table('coworker_trainings')->join('trainings', …)` — the query builder has no concept of model connections, so the JOIN happens on whatever connection runs it (here: tenant) → `tenant_X.trainings doesn't exist`. **Workaround (applied):** rewritten in Eloquent (`CoworkerTraining::query()->with(['coworker', 'training'])`) so each side resolves on its own connection. New code on tenancy-aware paths should prefer Eloquent over `DB::` — also a project-wide rule (see root `CLAUDE.md`).
...
- **Livewire `/livewire/update` doesn't replay tenancy middleware.** The web routes wrap pages in `InitializeTenancyByDomain` + `PreventAccessFromCentralDomains` at the group level, but Livewire's auto-registered `/livewire/update` endpoint goes through the `web` middleware group only. Its "persistent middleware" mechanism only re-runs classes explicitly added to an allowlist (default: `Authenticate`, `SubstituteBindings`, Sanctum/Jetstream auth). So on every Livewire action (clicks, computed re-eval, filter, modal submit…) **tenancy was not reinitialized** → default connection stayed `central` → any query on a tenant table (`Client::orderBy(...)->get()`, …) 500'd with `Base table not found in aeropaperasse_central`. The initial page render worked because that route does carry the tenancy middleware; the bug only surfaced on the *next* Livewire interaction. Tests didn't catch it because `TenantTestCase::setUp()` calls `tenancy()->initialize()` manually, masking the missing replay. **Fix:** `AppServiceProvider::boot()` calls `Livewire::addPersistentMiddleware([InitializeTenancyByDomain::class, PreventAccessFromCentralDomains::class, EnsureTenantMembership::class])`, so those three classes are re-applied to `/livewire/update` from the originating page's route.
